  ...
  return <motion.div initial={{
    ...
    duration: 0.5
      ...
      <div className="flex flex-col space-y-4 px-[30px]">
        {bookingOptions.map(option => <motion.button key={option.id} whileHover={{
        y: -2,
        boxShadow: '0 5px 10px rgba(255, 215, 0, 0.2)'
      }} whileTap={{
        y: 0
      }} onClick={() => handleSelectType(option.id)} className="booking-pill flex items-center justify-center">
            {option.icon}
